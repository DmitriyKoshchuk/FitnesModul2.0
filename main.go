...
func (t Training) TrainingInfo() InfoMessage {
	...
	return InfoMessage{
		...
		Distance:     t.distance(),   // Вызываем метод distance()
		Speed:        t.meanSpeed(),  // Вызываем метод meanSpeed()
		Calories:     t.Calories(),   // Если есть метод calories()
	}
}
...
func (r Running) Calories() float64 {
	// вставьте ваш код ниже
	speed := r.meanSpeed()
	return (CaloriesMeanSpeedMultiplier*speed + CaloriesMeanSpeedShift) * r.Weight * r.Duration.Hours()
}
...
func (w Walking) Calories() float64 {
	...
	return ((CaloriesWeightMultiplier*w.Weight + (math.Pow(speed, 2)/height)*CaloriesSpeedHeightMultiplier*w.Weight) * w.Duration.Hours())
}
